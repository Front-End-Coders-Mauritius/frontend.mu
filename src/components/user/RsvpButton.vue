...
<template>
  <div class="text-verse-100 font-medium py-16 w-full md:w-auto">
      ...
      <div v-else>
        <Transition mode="out-in" name="fade">
          <div v-if="rsvp_is_attending">
            <!-- Attending -->
            <rsvp-booking-confirmed @open="$emit('open')" @cancel-rsvp="unRsvpToMeetup()" :avatar_url="profile.avatar_url"
              :rsvp_loading="rsvp_loading" :rsvp_is_attending="rsvp_is_attending" />
          </div>
          <div v-else>
    ...
    <div v-else>
      <template v-if="!is_rsvp_closed">
        <a class="px-8 py-4 rounded-md text-lg font-bold inline-flex items-center justify-center shadow-sm bg-verse-700"
      ...
      <template v-else>
        <div
          class="relative w-full text-center md:w-auto inline-flex justify-center items-center rounded-md bg-white/60 px-8 py-4 text-lg font-semibold text-gray-900 ring-1 ring-inset ring-gray-300 focus:z-10">
          <span class="dark:text-verse-700 text-green-600 uppercase">RSVP Closed</span>
        </div>
      </template>
    </div>
  </div>
