  ...
  <div
    ...
    <div v-if="event.Date" class="">
      <span class="inline-flex rounded-lg p-3 ring-4 ring-white dark:ring-white/5" :class="isUpcoming(event.Date)
        ...
        : 'bg-verse-50 text-verse-600 dark:text-verse-400 font-bold dark:bg-slate-900/10'
        ">
    ...
    <div class="flex flex-col gap-4 md:gap-0">
      <h3 class="leading-2 text-xl md:text-2xl font-medium py-2 text-verse-500 dark:text-white">
        <a :href="`meetup/${event.id}`" class="w-[300px] md:w-96 focus:outline-none">
      ...
      <div class="flex gap-4 border-gray-100">
        <div class="flex gap-1 md:gap-0 items-center justify-start text-base font-medium text-gray-500">
          <IconGroup class="mr-1.5 h-[15px] w-[15px] flex-shrink-0 truncate text-gray-500" aria-hidden="true" />
        ...
        <div v-if="event.Venue"
          ...
          <IconLocation class="ml-[-1px] mr-1.5 h-4 w-4 flex-shrink-0 truncate text-gray-500" aria-hidden="true" />
          <div class="line-clamp-1 md:line-clamp-0">{{ event.Venue }}</div>
        </div>
        <div v-else></div>
      </div>
    </div>
    <span class="pointer-events-none absolute top-6 right-6 text-gray-300 group-hover:text-verse-500" aria-hidden="true">
