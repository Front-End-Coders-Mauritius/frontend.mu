<template>
  <button class="toggle hidden md:block" aria-pressed="false" :title="isDark ? 'Toggle Light Mode' : 'Toggle Dark Mode'"
    @click="toggleDark()">
          ...
          <svg aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 316 432">
            ...
            <defs>
              ...
              <radialGradient id="c" cx="0" cy="0" r="1"
                gradientTransform="matrix(165.9998 58.9999 -60.6938 170.7657 210 171)" gradientUnits="userSpaceOnUse">
...
onMounted(() => {
  ...
  const TOGGLE = () => {
    if (BUTTON) {
      BUTTON.setAttribute(
        "aria-pressed",
        localStorage["vueuse-color-scheme"] === "light" ? "true" : "false"
      );
    }
  };
...
.toggle {
  ...
  box-shadow: calc(var(--width) * 0) calc(var(--width) * 0.02) calc(var(--width) * 0.01) calc(var(--width) * -0.0025) hsl(210 10% 100% / 0.95),
...
.astrobear svg {
  ...
  transition: rotate calc(var(--speed) + (var(--dark, 0) * (var(--bear-speed) - var(--speed)))) calc(var(--bear-speed) * 0.4) linear,
