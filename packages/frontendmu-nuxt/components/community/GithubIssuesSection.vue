        ...
        <template v-for="issue in issues" :key="issue.number">
            <div class="border bg-verse-950 border-verse-400/50 p-3 rounded-md flex gap-2">
                <div class="flex items-center">
                    <img :src="githubProfileUrl(issue.author.login)" alt="" class="bg-red-500 w-8 h-8 rounded-full">
                </div>
                <div>
                    <div class="flex gap-1.5 items-center">
                        <Icon :name="issueIcon(issue.state as IssueState)" class="text-[#57ab5a]" />
                        <a :href="issue.url" target="_blank" class="text-verse-600 no-underline dark:text-verse-300">
                            {{ issue.title }}
                        </a>
                    </div>
